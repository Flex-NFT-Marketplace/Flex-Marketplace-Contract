...
namespace IERC721 {
    ...
    func baseURI() -> (baseURI_len: felt, baseURI: felt*) {
    }
}
...
@storage_var
func ERC721_old_collection() -> (old_collection: felt) {
}
...
func initializer{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    old_collection: felt, proxy_admin: felt
) {
    Proxy.initializer(proxy_admin);
    ERC721_old_collection.write(old_collection);

    let (name) = IERC721.name(contract_address=old_collection);
    let (symbol) = IERC721.symbol(contract_address=old_collection);
    ERC721.initializer(name, symbol);
    ERC721Enumerable.initializer();

    let (owner) = IERC721.owner(contract_address=old_collection);
    Ownable.initializer(owner);

    let (baseURI_len, baseURI) = IERC721.baseURI(contract_address=old_collection);
...
func mint{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let (old_collection) = ERC721_old_collection.read();
    ...
    ERC721Enumerable._mint(caller, token_id);
    return ();
}
