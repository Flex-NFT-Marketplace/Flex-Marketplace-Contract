...
pub mod ERC721Component {
    ...
    use base64_nft::nft_render::NftRender::{INftRenderDispatcherTrait, INftRenderDispatcher};
    ...
    > of interface::IERC721Metadata<ComponentState<TContractState>> {
        ...
        fn token_uri(self: @ComponentState<TContractState>, token_id: u256) -> ByteArray {
            self._require_owned(token_id);
            let render = INftRenderDispatcher { contract_address: self.render.read() };
            let uri = render.tokenURI(token_id);
            return format!("{}", uri);
        }
    }
    ...
    fn _check_on_erc721_received(
        ...
        if src5_dispatcher.supports_interface(interface::IERC721_RECEIVER_ID) {
            ...
            src5_dispatcher.supports_interface(openzeppelin::account::interface::ISRC6_ID)
        }
    }
}
