...
use tests::utils::{
    setup, initialize_test, OWNER,ACCOUNT1,
    ACCOUNT2,
};
...
fn test_add_strategy_success() {
    ...
    start_prank(
        CheatTarget::One(dsp.execution_manager.contract_address),
        OWNER()
    );
...
fn is_strategy_whitelisted() {
    ...
    let is_whitelisted = dsp.execution_manager.is_strategy_whitelisted(
        ACCOUNT1()
    );
...
fn get_whitelisted_strategies_count() {
    ...
    assert!(
        whitelisted_count == 2,
        "COUNT ERROR: Expected 2 but found {}",
        whitelisted_count
    );
}
